...
const RestartButton: React.FC<RestartButtonProps> = ({ className = "", variant = 'icon' }) => {
  ...
  const handleRestartServer = async () => {
    if (!window.confirm('Are you sure you want to restart the server?')) return;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        alert('Server restart triggered successfully!');
      }
    } catch (err) {
  ...
  if (variant === 'full') {
    return (
      <button
        ...
        className={`w-full bg-gray-50 hover:bg-gray-100 text-gray-800 p-6 rounded-2xl flex items-center justify-between border-2 border-gray-100 transition-all group ${restartLoading ? 'opacity-50 cursor-not-allowed' : ''} ${className}`}
      >
        ...
        <RefreshCw className={`text-gray-300 ${restartLoading ? 'animate-spin' : ''}`} size={20} />
      </button>
    );
  }
